...
//   }
    ...
    const handleClick = (userId) => {
        navigate('/viewFeedback', { state: { userId: userId } });
    };
...
/>          
    ...
    <Thead>
      <Tr>
        ...
        <Th style={{ fontSize:"15px" }} >View FeedBack </Th>
      </Tr>
    </Thead>
    <Tbody>
     { user? (user.map((u)=>(
     ...
     <Td>{u.postDate}
         ...
         <Td>{u.location}
            ...
            <Td> {u.votesByUser?.length}
            </Td>  
         <Td>{u.status}</Td>
         <Td>{u.estimateDate}</Td>
         <Td>{u.actualDate}</Td>
       
         <Td> <Button onClick={() => handleClick(u._id)}> View FeedBack </Button> </Td>
